    ...
    <div class="w3-container">
     {{-- {{print_r($harga->where('kamar', 'Anggrek 2')[1]['harga'])}} --}}
      <ul style="list-style:none;margin-left:-54px">
        ...
        <li>(0333) 845844</li>
        <li><hr></li>
        <li>Check In: {{date('D,d-m-Y H:i',
          strtotime($invoice->reservation->created_at->timezone('Asia/Jakarta')))}}
          by {{$invoice->reservation->user->name}}
        </li>
        <li>Check Out : {{date('D,d-m-Y H:i',
          strtotime($invoice->created_at->timezone('Asia/Jakarta')))}}
          by {{$invoice->reservation->checkout_user->name}}
        </li>
      </ul>
    </div>
